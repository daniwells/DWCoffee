        ...
        <Container styleProp={`flex-col overflow-hidden -z-10 ${display} transition-all duration-500`} >
            <section className='bg-customBlack flex items-center justify-center p-5 h-1/4 w-full border-t-2 border-t-customYellowLight  ' >
                <section className='text-white flex space-x-10'>
                    ...
                    <Link className="cursor-pointer hover:text-customYellowLight" to={'/'} >REGISTER</Link>
                </section>
            </section>
            <nav className=" mt-5 flex flex-col" >
                ...
                <LinkMennu text={<><SiHomeassistantcommunitystore className='text-customBlack'/><p>Restaurants</p></>} to="/" currentLocation={location.pathname === "/restaurants" ? 'bg-gray-100' : false}/>
                <LinkMennu text={<><PiNewspaperClippingFill className='text-customBlack'/><p>News</p></>} to="/" currentLocation={location.pathname === "/news" ? 'bg-gray-100' : false}/>
            </nav>
