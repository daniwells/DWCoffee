function LongTextInputApply({handleChange, text, placeholder, data_group, key}){
    ...
    return (
        <section>
            <textarea data-group={data_group} name="about" onChange={sendValue} cols="5" rows="10" placeholder={placeholder} className='bg-transparent px-5 py-3 border-2  outline-none focus:border-black text-lg text-opacity-80 font-semibold border-black border-opacity-70 w-full rounded-lg placeholder:text-opacity-60 placeholder:text-black placeholder:arimo placeholder:font-bold placeholder:text-xl'></textarea>
