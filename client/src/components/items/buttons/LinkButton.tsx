...
const LinkButton: React.FC<LinkButtonProps> = ({text, styleProp, to, currentLocation}) => {
    let styles: Record<string, string> = {
        ...
        "about":"flex justify-center w-2/4 text-sm py-1 px-5 rounded-2xl font-archivoBlack bg-customBrown text-white opacity-75 hover:opacity-100 hover:bg-customBrown transition-all duration-700 ease-in-out",
        "transparent":" text-center text-base font-arimo text-customBlack2 transition-all duration-700 ease-in-out hover:text-customBrown w-full ",
        ...
        "coffee_card":"text-center w-32 text-md font-archivoBlack py-1 px-5 rounded-2xl bg-customBrown text-white opacity-75 hover:opacity-100 hover:bg-customBrown transition-all duration-700 ease-in-out",
    }
